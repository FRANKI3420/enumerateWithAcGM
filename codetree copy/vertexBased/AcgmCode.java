...
public class AcgmCode
    ...
    @Override
    public ObjectFragment[] computeCanonicalCode(int labels_length) {
        ObjectFragment[] codeList = new ObjectFragment[labels_length];
        for (int i = 0; i < labels_length; i++) {
            codeList[i] = new AcgmCodeFragment((byte) (labels_length - 1 - i), 0);
        }
        return codeList;
    }
    ...
    @Override
    public boolean computeCanonicalCode(Graph g,ObjectFragment[] target) {
        final int n = g.order();
        ...
        for (int depth = 1; depth < n; ++depth) {
            ...
            code.add(maxFrag);
            if(!code.get(depth).equals(target[depth])){
                return false;
            }
    ...
    @Override
    public boolean isCanonical(Graph g ,ObjectFragment[] c) {
    ...
    @Override
    public Graph generateGraphAddElabel(ObjectFragment[] code, int id) {
        byte[] vertices = new byte[code.length];
        byte[][] edges = new byte[code.length][code.length];
        int index = 0;
